...
public class RaidCharStatsPanel : MonoBehaviour
{
    ...
    {
        maxHealth.text = string.Format("{0:0.0}/{1:0.0}",
            character.GetPairedAttribute(AttributeType.HitPoints).CurrentValue,
            character.GetPairedAttribute(AttributeType.HitPoints).ModifiedValue);

        stressLevel.text = string.Format("{0:0.0}/{1:0.0}",
